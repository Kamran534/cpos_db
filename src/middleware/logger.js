...
const colorize = {
  ...
  ip: (ip) => {
    return `${colors.dim}${ip}${colors.reset}`;
  },
  
  date: (date) => {
    return `${colors.dim}${date}${colors.reset}`;
  },
};

// Custom token for colored output
morgan.token('colored-method', (req) => colorize.method(req.method));
morgan.token('colored-status', (req, res) => {
  const status = res.statusCode || '-';
  return colorize.status(status);
});
morgan.token('colored-url', (req) => colorize.url(req.originalUrl || req.url));
...
// Custom format string with colored output
// Use morgan's built-in :response-time token to ensure it's calculated
const format = ':colored-date | :colored-ip | :colored-method :colored-url | :colored-status | :response-time';

// Create a custom stream that colorizes the response time
const coloredStream = {
  write: (message) => {
    // Colorize the response time in the message
...
// Create colored logger middleware with response-time
const coloredLogger = morgan(format, {
  skip: (req, res) => {
    // Skip logging for health checks or specific paths if needed
    // You can add conditions here, e.g., req.path === '/health'
    return false;
  },
